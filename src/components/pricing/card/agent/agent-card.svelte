...
{#if block}
  ...
  <div class="glass-border relative h-full">
    <div
      class={cn(
        'glass-card relative w-full md:h-full',
...
box-shadow:{!tag ? '' : '0px 0px 40px rgba(104, 83, 214, 0.24)'}"
    >
      ...
      <div
        class={cn('p-5 md:p-6', 'flex w-full flex-shrink-0 flex-grow-0 flex-col justify-between')}
      >
            ...
            {#if description}
              {#if typeof description != 'string' && description.content}
                {#each description.content as content}
                  <p
                    class={cn(paragraph, 'mt-3 text-sm/snug font-medium tracking-wide opacity-64')}
                  >
...
<style>
  .glass-border {
    padding: 1px; /* Figma border thickness */
    border-radius: 24px;
    background: rgba(250, 250, 255, 0.04);
  }

  .glass-card {
    background: linear-gradient(0deg, rgba(250, 250, 255, 0.04), rgba(250, 250, 255, 0.04)),
      rgba(10, 9, 20, 0.3);
