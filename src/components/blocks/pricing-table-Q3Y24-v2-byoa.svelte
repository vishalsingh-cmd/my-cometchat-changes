<script lang="ts">
  ...
  import PricingTableLineQ3Y24 from './pricing-table-line-Q3Y24-byoa/pricing-table-line-Q3Y24.svelte';
  import scrollDirection from '$lib/stores/scroll-direction';
  import TitleSection from './title-section.svelte';
  import Dropdown from '$components/dropdown.svelte';
  import PricingTableLineQ3Y24Portrait from './pricing-table-line-Q3Y24-byoa/pricing-table-line-Q3Y24-portrait.svelte';
  ...
  let plans = Object.keys($pricingValuesbyoa);
          ...
          {#each Object.keys($pricingValuesbyoa) as plan, index}
            ...
            >
              ...
              <p class="px-4 text-center text-lg/normal font-medium tracking-wide opacity-74">
                {#if plan !== 'Build'}
                  {$pricingValuesbyoa[plan].price}
                {:else}
                  Free Forever
                {/if}
                {#if $pricingValuesbyoa[plan].price.startsWith('$') && plan !== 'Build'}
                  <span class="-ml-[4px]">/month</span>
                {/if}
              </p>
              <p class="text-md font-medium tracking-wide opacity-74">
                {$isBilledAnnualy &&
                $pricingValuesbyoa[plan].price.startsWith('$') &&
                plan !== 'Build'
                  ? 'Billed annually'
                  : ''}
              </p>
