<script lang="ts">
  ...
  const DynamicCallout = tv({
    slots: {
      base: [
        'animate-show-callout absolute bottom-[20px] sm:bottom-[100px] left-0 right-0 m-auto flex items-center justify-center transition-all duration-500 ease-out '
      ],
      wrapper: [
  ...
  {#if visible}
    <div class={base({ class: className })}>
      <div bind:this={wrapperEl} class={wrapper({ class: className })}>
        ...
        <!-- {/if} -->
        <div
          class="absolute bottom-[20px] left-0 right-0 top-1/2 m-auto h-full w-full -translate-x-1/2 -translate-y-1/2 animate-ping rounded-xl sm:bottom-[100px]"
        />
      </div>
    </div>
  {/if}
