<script lang="ts">
  ...
  import CometchatLogo from '$components/cometchat-logo.svelte';
...
{#if data}
  <footer data-theme="light" class="bg-[#F5F5F5] text-gray-12">
    <div class="container mx-auto px-container">
      ...
      {#if data.columns}
                  ...
                  <div class="flex flex-col gap-6 first:pt-4">
                    {#each column.groups as { title, links }}
                      <div>
                        {#if title}
                          <p
                            class="mb-4 text-sm/normal font-semibold tracking-widest text-gray-12/60"
                          >
                            {title}
                          </p>
                        {/if}
        ...
        <!-- main links - Desktop -->
        <div class="hidden pb-10 sm:grid sm:grid-cols-2 sm:gap-20 lg:grid-cols-4">
